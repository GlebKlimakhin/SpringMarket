package com.example.corelib.configurations.jwt;
...
public class JWTAuthenticationFilter extends OncePerRequestFilter {
    ...
    private UsernamePasswordAuthenticationToken createToken(String authorizationHeader) throws ExpiredJwtException {
        ...
        if (userInfo.getRoles() != null && !userInfo.getRoles().isEmpty()) {
            userInfo.getRoles().forEach(role -> authorities.add(new SimpleGrantedAuthority(role)));
        }
    ...
    private boolean authorizationHeaderIsInvalid(String authorizationHeader) {
        return authorizationHeader == null
                || !authorizationHeader.startsWith("Bearer ") || redisRepository.checkToken(authorizationHeader);
    }
}
